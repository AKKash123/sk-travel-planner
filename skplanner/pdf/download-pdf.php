    ...
    if (file_exists($imgFile)) {
        ...
        if ($ext === 'jpg' || $ext === 'jpeg' || $ext === 'png') {
            ...
            if ($is !== false) {
                ...
                $ix = (210 - $dw) / 2;
                $imgType = ($ext === 'jpeg') ? 'jpg' : $ext;
                $pdf->Image($imgFile, $ix, $pdf->GetY(), $dw, $dh, $imgType);
...
// Bullet item helper
function bulletItem($p, $text, $r = 13, $g = 115, $b = 119) {
    $x = $p->lMargin;
    $width = $p->w - $p->rMargin - $p->lMargin;
    $yBullet = $p->GetY() + 1.8;
    $p->SetFillColor($r, $g, $b);
    $p->Rect($x + 1, $yBullet, 2.2, 2.2, 'F');
    $p->SetX($x + 5);
    $p->MultiCell($width - 5, 6, S($text));
}
